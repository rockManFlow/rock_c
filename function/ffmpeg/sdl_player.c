...
#include "../../include/ffmpeg/win/libavutil/imgutils.h"
...
int main(int argc, char* argv[])
{
    ...
    //SDL_Window就是使用SDL的时候弹出的那个窗口
    SDL_Window *screen;
    //SDL_Renderer用于渲染SDL_Texture至SDL_Window
    SDL_Renderer* sdlRenderer;
    //SDL_Texture用于显示YUV数据。一个SDL_Texture对应一帧YUV数据。
    SDL_Texture* sdlTexture;
    //SDL_Rect用于确定SDL_Texture显示的位置。注意：一个SDL_Texture可以指定多个不同的SDL_Rect，
    // 这样就可以在SDL_Window不同位置显示相同的内容（使用SDL_RenderCopy()函数）
    SDL_Rect sdlRect;

    FILE *fp_yuv;

//    avcodec_register_all();//av_register_all使用avcodec_register_all代替
