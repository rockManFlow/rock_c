 ...
 * excel2003--可以导出单个sheet,最大只能是65535
 */
public class ExcelPoiUtil {
    ...
    public ExcelPoiUtil(HSSFWorkbook workbook) {
        this.workbook = workbook;
        this.sheet = workbook.createSheet();
    }

    public ExcelPoiUtil(HSSFWorkbook workbook, HSSFSheet sheet) {
        this.workbook = workbook;
        this.sheet = sheet;
    }
    ...
    private void createColumnData(String[] columnHeader, List<?> dataList, HttpServletResponse response, String fileName) {
        ...
        if (!dataList.isEmpty()) {
            ...
            sheet = createColumnData(sheet, rowNO, 400, columnData, 50000);
        }
     ...
     */
    private HSSFSheet createColumnData(HSSFSheet sheet, int rowNO, int rowHeight, String[][] columnData, int maxValue) {
    ...
    private void createColumnDataDesc(HSSFSheet sheet, int numNO, int rowNO, int currRowNO, int rowHeight,
                                      String[][] columnData, HSSFCellStyle cellStyle) {
     ...
     */
    private void createColumnHeader(HSSFSheet sheet, int rowNO, int rowHeight, String[] columnHeader) {
     ...
     */
    private void createColumnHeader(HSSFSheet sheet, int rowNO, int rowHeight, String[] columnHeader, int fontSize,
     ...
     * @param align 字体水平位置 (center中间 right右 left左)
     */
    private HSSFCellStyle createCellFontStyle(HSSFWorkbook workbook, int fontSize, String fontWeight, String align) {
        if (workbook == null) {
            System.out.println("Excel HSSFWorkbook FontStyle is not set");
            return null;
        }
    ...
    private void exportExcelStream(HttpServletResponse response, String fileName){
        ...
        try {
            ...
            System.out.println("exportExcelStream exception");
        }finally {
            if(os!=null){
                try {
                    os.close();
                } catch (IOException e) {
                    System.out.println("exportExcelStream close exception");
                }
            }
        }
    }
