...
### 16、static关键字的作用--静态
1. 修饰局部变量，成为静态变量，存储在静态区。作用域在局部，每次调用该作用域，该值会一直增加
...
#### 数据类型
long‌：在32位系统上是4字节，在64位是8字节
long long：在32和64位系统上都是8字节，这就是这俩的区别。

sprintf：sprintf(message, "xxx %d"", clientfd);//将格式化字符串存储到message中
printf：函数将输出文本打印到标准输出（通常是控制台）
perror：用于将对应的错误信息输出到标准错误流（stderr）中。


线程、队列、锁、MQ 、list等集合类 在C/C++中也有对应实现--